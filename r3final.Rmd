...
bids_dir <- here() # not necessary to use the `path()` function. `here()` suffices.
deriv_dir <- here("derivatives")
data_dir <- here("derivatives", "pattern_similarity") # use the comma instead of a file separator to make it non-OS specific
...
# Subject list
subj_list <- 
  read_tsv(here("participants.tsv"), col_types = "cicc") %>% 
...
estimate_id <- "tmaps"
#flavor_id <- ""  #This does nothing in the code. No need to include it.
```
...
simi_list_rtv <- 
  map_chr(
  subj_list, ~ 
    glue(data_dir, .Platform$file.sep, "sub-", .x, .Platform$file.sep,"roi_pearson_similarity", .Platform$file.sep, "sub-", .x, "_space-T1w_task-retrieval_desc-{preproc_id}-{estimate_id}-zr_similarity.tsv")
  )  # Path() is unnecessary. And a single glue function can be used to clean up the path concatenation.
# .Platform$file.sep will give the appropriate file separator based on the user's OS

# map 2
labels_list <-
  map_chr(
    subj_list, ~ 
      glue(data_dir, .Platform$file.sep, "sub-", .x, .Platform$file.sep, "beh_labels", .Platform$file.sep, "sub-", .x, "_labels_pairwise.tsv")
  ) # Again, path() is unnecessary. And a single glue function can be used to clean up the path concatenation.
...
simi_rtv_unlabel <- simi_rtv_non_within_pair %>% 
  modify_at(c("i1_item", "i2_item", "i1_obj", "i2_obj"), as.integer) %>% 
  rbind(., simi_rtv_non_within_pair)  # Can use a purrr function, modify_at(), instead of changing individual variables to integers. Then pipe to rbind().
```
...
simi_rtv <-  simi_rtv_unlabel %>% 
  ...
  )  #Since all the i2_item values are linear combinations of the i1_item values, this could be written as a function of some sort. For instance, create a vector of the values of i1_item that are "between_pair" and the i2_item is equal to that value + 1.
```
...
kable(dat_rtv_all, digits = 2)  # specify digits for readability
```
